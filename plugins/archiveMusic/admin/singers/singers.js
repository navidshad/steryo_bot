var show = function(userid){
    fn.userOper.setSection(userid, fn.mstr.arc.sections.s, true);
    var list = [];
    var back = fn.mstr.arc['back'];
...
var listingalbums = function(userid, singer){
    var list = [];
    var back = fn.mstr.arc.btns['singers'].back;
    var mess = singer;
    //get list
    fn.api.getalbums(singer, (albums) => {
        if(albums.length > 0){
            var section = fn.mstr.arc.sections.a + '-' + singer;
...
var routting = function(message, speratedSection){
    ...
    //show singers list
    if (text === fn.mstr.arc.btns['singers'].lable || text === fn.mstr.arc.btns['singers'].back)
        show(message.from.id);

    //show singer's album
    else if (speratedSection[speratedSection.length-1] === fn.mstr.arc.sections.s)
        listingalbums(message.from.id, message.text);

    //show singer's album
    else if (speratedSection[speratedSection.length-1].includes(fn.mstr.arc.sections.a)){
